...
long long  import_base64_prikey_handler(
    const char *buf, int buf_size);

 char * export_new_prikey_base64();

 long long  import_prikey_handler_from_hex(
    const char *str);

 char * export_new_prikey_to_hex(long long pkey);

 char * export_mnemonic_from_prikey_handler(
    long long pkey);

 long long  import_prikey_handler_from_mnemonic(
    const char *mnemonic);
